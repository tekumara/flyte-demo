...
def test_extract_reference_data():
    ...
    assert len(data) == 3
    assert data.routes[0] == {'airline': 'C7', 'codeshare': '', 'equipment': 'EMB', 'from': 'MAO', 'stops': '0', 'to': 'CIZ'}
    assert data.airlines["0B"] == "Blue Air"
    assert data.airports["AAA"] == {'airport-name': 'Anaa                                    ', 'latitude': '-17.3542', 'longitude': '-145.4961'}
